...
const elements = {
  ...
  showGoalIntervals: document.querySelector("#show-goal-intervals"), refreshGoalIntervals: document.querySelector("#refresh-goal-intervals"), goalIntervalsStatus: document.querySelector("#goal-intervals-status"), goalIntervalsContent: document.querySelector("#goal-intervals-content"),
  ...
  performanceMonthFilterPanel: document.querySelector("#performance-month-filter-panel"),
  performanceMonthFilter: document.querySelector("#performance-month-filter"),
  applyPerformanceMonthFilter: document.querySelector("#apply-performance-month-filter"),
  clearPerformanceMonthFilter: document.querySelector("#clear-performance-month-filter"),
  performanceMonthFilterStatus: document.querySelector("#performance-month-filter-status"),
  teamGoalInsightsContent: document.querySelector("#team-goal-insights-content"),
  refreshTeamGoalInsights: document.querySelector("#refresh-team-goal-insights"),
...
function resolveStableApiUsageDaily(observability = {}) {
  ...
  const activeWindowKey = daily.windowKey || persisted.windowKey || "";
  const validCached = cached && (!activeWindowKey || cached.windowKey === activeWindowKey) ? cached : null;
  const candidates = [daily, persisted, validCached].filter(Boolean);
  ...
  const normalized = {
    ...
    trackedNetworkRequests: Number(best?.trackedNetworkRequests ?? best?.networkRequests ?? 0),
    providerReportedRequests: Number(best?.providerReportedRequests || 0),
    unattributedNetworkRequests: Number(best?.unattributedNetworkRequests || 0),
...
function mergedApiUsageEndpointRows(daily = {}, observability = {}) {
  const rows = new Map();
  const addRows = (source = {}) => {
    for (const [endpoint, metrics] of Object.entries(source || {})) {
      const current = rows.get(endpoint) || {
        networkRequests: 0,
        cacheHits: 0,
        cacheMisses: 0,
        pendingHits: 0,
      ...
      current.networkRequests = Math.max(Number(current.networkRequests || 0), Number(metrics.networkRequests || metrics.count || 0));
      current.cacheHits += Number(metrics.cacheHits || 0);
      current.cacheMisses += Number(metrics.cacheMisses || 0);
      current.pendingHits += Number(metrics.pendingHits || 0);
      current.negativeCacheHits += Number(metrics.negativeCacheHits || 0);
      current.failures = Math.max(Number(current.failures || 0), Number(metrics.failures || metrics.errors || 0));
      rows.set(endpoint, current);
    }
  };
  addRows(daily.endpoints);
  addRows(observability.endpoints);
  const unattributed = Number(daily.unattributedNetworkRequests || 0);
  if (unattributed > 0) {
    rows.set("Cuenta API-Football (sin ruta local)", {
      networkRequests: unattributed,
      cacheHits: 0,
      cacheMisses: 0,
      pendingHits: 0,
      negativeCacheHits: 0,
  ...
  return [...rows.entries()]
    .sort(([, left], [, right]) => (
      Number(right.networkRequests || 0) - Number(left.networkRequests || 0)
      || Number(right.cacheHits || 0) - Number(left.cacheHits || 0)
    ))
    .slice(0, 12);
}
...
function renderApiUsageAdmin(runtime = null, { loading = false, error = "" } = {}) {
  ...
  const trackedUsed = Number(daily.trackedNetworkRequests ?? displayedUsed);
  const unattributedUsed = Number(daily.unattributedNetworkRequests || 0);
  ...
  const endpointRows = mergedApiUsageEndpointRows(daily, observability);
  ...
  elements.apiUsageSummary.innerHTML = `
    <article><span>API-Football reales</span><strong>${numberLabel(displayedUsed)}</strong><small>Total confirmado por el proveedor - 5 p.m. PT</small></article>
    ...
    <article><span>Cache del sistema</span><strong>${displayValue(cacheHitRate)}%</strong><small>${numberLabel(observability.cacheHits, "0")} hits - no consumen cuota</small></article>
  ...
  elements.apiUsageTable.innerHTML = `
    <p class="account-note">${loading ? "Actualizando conteo..." : error ? escapeHtml(error) : `Conteo diario: ${escapeHtml(dailyWindow)}. ${numberLabel(trackedUsed, "0")} llamadas atribuidas a rutas del sistema${unattributedUsed > 0 ? ` y ${numberLabel(unattributedUsed, "0")} adicionales reportadas por la cuenta API-Football` : ""}. El boton solo lee el contador; no consume API-Football.`}</p>
    ${endpointRows.length ? `<div class="api-usage-table__scroll"><table>
      <thead><tr><th>Endpoint</th><th>API reales</th><th>Cache hits</th><th>Cache miss</th><th>Dedupe</th><th>Fallos</th></tr></thead>
...
function normalizedCompetitionName(value) {
  return String(value || "").normalize("NFD").replace(/[\u0300-\u036f]/g, "").toLowerCase()
    .replace(/\b(uefa|conmebol|clasificacion|qualification|qualifying)\b/g, " ")
    .replace(/[^a-z0-9]+/g, " ").trim();
}

function positiveCompetitionForFixtures(league, fixtures, rows) {
  ...
  return rows.find((row) => row.evaluated >= 3 && row.winRate >= 60
...
function renderFixtureData() {
  ...
  elements.refreshGoalIntervals.disabled = false;
  elements.showGoalHalf.disabled = state.isLoadingGoalHalf;
  elements.showGoalIntervals.disabled = state.isLoadingGoalHalf;
  const savedGoalHalf = state.goalHalfByFixture.get(fixture.id);
  if (savedGoalHalf) { renderGoalHalfWithPick(savedGoalHalf); renderGoalIntervals(savedGoalHalf); }
  ...
  if (!savedGoalHalf) { elements.goalIntervalsStatus.className = "status-badge status-badge--unavailable"; elements.goalIntervalsStatus.textContent = "No disponible"; elements.goalIntervalsContent.innerHTML = '<div class="research-empty">Pulsa “Actualizar datos” para comparar los rangos de gol de ambos equipos; después usa Mostrar u Ocultar.</div>'; }
  elements.goalHalfContent.hidden = true;
  elements.goalIntervalsContent.hidden = true;
  if (savedGoalHalf) { elements.showGoalHalf.textContent = "Mostrar"; elements.showGoalHalf.classList.remove("button--ready"); }
  if (savedGoalHalf) { elements.showGoalIntervals.textContent = "Mostrar"; elements.showGoalIntervals.classList.remove("button--ready"); }
  elements.showGoalHalf.setAttribute("aria-expanded", "false");
  elements.showGoalIntervals.setAttribute("aria-expanded", "false");
...
function renderResearchModuleDetail(moduleKey, research) {
  ...
  if (moduleKey === "standings") {
    ...
    const matchDivision = (match) => match?.division || match?.competition || match?.leagueName || match?.league || match?.tournament || null;
    const matchRows = (team, matches) => (matches || []).map((match) => [escapeHtml(team), displayValue(match.date), displayValue(match.opponent), displayValue(matchDivision(match)), displayValue(match.venue), `<strong>${displayValue(match.goalsFor)}–${displayValue(match.goalsAgainst)}</strong>`, displayValue(match.result)]);
    const analysis = evaluateResearchRecommendation("statsForm", research);
    content = `<div class="team-stat-grid">${researchTeamStats(research.homeTeam.name, [["Goles a favor", module.homeGoalsFor], ["Goles en contra", module.homeGoalsAgainst], ["Tasa de victoria", module.homeWinRate === null ? null : `${module.homeWinRate}%`], ["Porterías a cero", module.homeCleanSheets]])}${researchTeamStats(research.awayTeam.name, [["Goles a favor", module.awayGoalsFor], ["Goles en contra", module.awayGoalsAgainst], ["Tasa de victoria", module.awayWinRate === null ? null : `${module.awayWinRate}%`], ["Porterías a cero", module.awayCleanSheets]])}</div>${detailTable(["Equipo", "Fecha", "Rival", "División", "Sede", "Marcador", "Resultado"], [...matchRows(research.homeTeam.name, module.homeLastMatches), ...matchRows(research.awayTeam.name, module.awayLastMatches)])}${renderRecentFormSuggestion(analysis, research.homeTeam.name, research.awayTeam.name)}`;
...
function performanceHistoryData() {
  return {
    picks: filterPicksByFixtureMonth(state.savedPicks, state.performanceMonthFilter),
    parlays: filterParlaysByFixtureMonth(state.savedParlays, state.performanceMonthFilter)
  };
}

function performanceMonthLabel(month = state.performanceMonthFilter) {
  if (!month) return "todo el historial";
  const parsed = new Date(`${month}-01T12:00:00`);
  return Number.isNaN(parsed.getTime()) ? month : new Intl.DateTimeFormat("es-MX", { month: "long", year: "numeric" }).format(parsed);
}

function updatePerformanceMonthStatus() {
  if (!elements.performanceMonthFilterStatus) return;
  elements.performanceMonthFilterStatus.textContent = state.performanceMonthFilter
    ? `Mostrando únicamente ${performanceMonthLabel()}.`
...
function showPerformancePicksDialog({ title, subtitle, picks = [] }) {
  if (!elements.originPicksDialog) return;
...
function showOriginPicksDialog(origin, result) {
  ...
  const row = calculateOriginPerformance(history.picks, history.parlays).find((item) => item.origin === origin);
...
function showCompetitionPicksDialog(key, result) {
  const history = performanceHistoryData();
  const row = calculateCompetitionPerformance(history.picks, history.parlays).find((item) => item.key === key);
  if (!row) return;
  const isWon = result === "won";
  const picks = row[isWon ? "wonPicks" : "lostPicks"] || [];
  showPerformancePicksDialog({
    title: row.competition,
    subtitle: `${isWon ? "Picks ganados" : "Picks perdidos"} · ${picks.length} · ${performanceMonthLabel()}`,
    picks
  });
}

function renderTeamGoalInsights() {
  if (!elements.teamGoalInsightsContent) return;
  const report = calculateParlayTeamGoalLeaders(state.savedParlays, { limit: 5 });
  elements.refreshTeamGoalInsights.disabled = state.savedParlays.length === 0;
  if (!report.fixturesUsed) {
    elements.teamGoalInsightsContent.innerHTML = '<div class="saved-empty"><h3>Sin marcadores reglamentarios disponibles</h3><p>Guarda y actualiza parlays finalizados para construir este resumen. No se inventan resultados ausentes.</p></div>';
    return;
  }
  const ranking = (title, rows, metric, tone) => `<section class="team-goal-ranking team-goal-ranking--${tone}"><header><div><span>${escapeHtml(rows[0]?.side || "Condición")}</span><h3>${escapeHtml(title)}</h3></div><small>Promedio por encuentro</small></header><div class="team-goal-ranking__list">${rows.map((row, index) => `<article><b>${index + 1}</b><div><strong>${escapeHtml(row.team)}</strong><small>${escapeHtml(row.side)} · ${row.matches} ${row.matches === 1 ? "partido" : "partidos"}</small></div><span><strong>${displayValue(row.average)}</strong><small>${row[metric]} goles</small></span></article>`).join("")}</div></section>`;
  elements.teamGoalInsightsContent.innerHTML = `
    <div class="team-goal-insights-summary"><span>Encuentros únicos analizados</span><strong>${report.fixturesUsed}</strong><small>Los picks repetidos del mismo fixture cuentan una sola vez.</small></div>
    <div class="team-goal-insights-grid">
      ${ranking("Más goleadores como local", report.scorers.home, "goalsFor", "positive")}
      ${ranking("Más goleadores como visitante", report.scorers.away, "goalsFor", "positive")}
      ${ranking("Más goleados como local", report.conceded.home, "goalsAgainst", "negative")}
      ${ranking("Más goleados como visitante", report.conceded.away, "goalsAgainst", "negative")}
    </div>`;
}
...
function renderOriginPerformance() {
  ...
  const renderPickTypes = (result) => {
    ...
    return filtered.map((row) => { const rate = row.total ? row.won / row.total * 100 : null; return `<article class="pick-type-summary__item pick-type-summary__item--${result}"><span>${escapeHtml(row.type)}${performanceSignalBadge(rate, row.total)}</span><strong>${row[result]}</strong></article>`; }).join("");
  };
...
function renderYellowCards(result = {}) {
  const status = result.status === "available" ? "Disponible" : result.status === "partial" ? "Parcial" : "No disponible";
  elements.yellowCardsStatus.className = `status-badge status-badge--${statusClass(status)}`;
  elements.yellowCardsStatus.textContent = status;
  if (result.status === "not_available") {
    elements.yellowCardsContent.innerHTML = `<div class="research-empty">${escapeHtml(result.warning || "Tarjetas amarillas no disponible.")}</div>`;
    return;
  }
  const team = (data, label) => `<article class="corner-team"><div><small>${escapeHtml(label)}</small><h3>${displayValue(data.yellowCardsForAvg)} amarillas</h3></div><div class="team-goal-kpis"><span>A favor<strong>${displayValue(data.yellowCardsForAvg)}</strong></span><span>Rivales reciben<strong>${displayValue(data.yellowCardsAgainstAvg)}</strong></span><span>Muestra<strong>${displayValue(data.useful, 0)}</strong></span></div><small>${displayValue(data.useful, 0)} oficiales usados · ${displayValue(data.excludedFriendlies, 0)} amistosos excluidos</small><small>${escapeHtml(data.competitions?.join(" · ") || "Competición no informada")}</small></article>`;
  const projection = result.projection || {};
  const sampleRows = [
    ...(result.teams?.home?.fixturesUsed || []).map((row) => [selectedFixture()?.home || "Local", row.date, row.opponent, row.venue, row.yellowCardsFor, row.yellowCardsAgainst]),
    ...(result.teams?.away?.fixturesUsed || []).map((row) => [selectedFixture()?.away || "Visitante", row.date, row.opponent, row.venue, row.yellowCardsFor, row.yellowCardsAgainst])
  ];
...
function renderGoalIntervals(result = {}) {
  ...
  elements.goalIntervalsStatus.textContent = status;
  const comparison = result.intervalComparison;
  if (!comparison?.rows?.length) {
    elements.goalIntervalsContent.innerHTML = `<div class="research-empty">${escapeHtml(result.warning || "No existen eventos suficientes para comparar rangos de tiempo.")}</div>`;
    return;
  }
  const fixture = selectedFixture();
  const homeName = fixture?.home || "Local";
  const awayName = fixture?.away || "Visitante";
  const rows = comparison.rows.map((row) => [
    row.label,
    `${displayValue(row.homeGoals, 0)} goles · ${displayValue(row.homeWeightedRate, 0)}%`,
...
async function loadGoalHalf(forceRefresh = false) {
  ...
  const intervalsWereHidden = elements.goalIntervalsContent.hidden;
  state.isLoadingGoalHalf = true;
  elements.refreshGoalHalf.disabled = true;
  elements.refreshGoalIntervals.disabled = true;
  elements.showGoalHalf.disabled = true;
  elements.showGoalIntervals.disabled = true;
  ...
  elements.goalIntervalsStatus.className = "status-badge status-badge--processing";
  elements.goalIntervalsStatus.textContent = forceRefresh ? "Actualizando" : "Procesando";
  try {
    ...
    renderGoalIntervals(result);
    if (forceRefresh) elements.goalHalfContent.hidden = wasHidden;
    if (forceRefresh) elements.goalIntervalsContent.hidden = intervalsWereHidden;
    ...
    renderGoalIntervals({ status: "not_available", warning: error.message });
    elements.goalHalfContent.hidden = forceRefresh ? wasHidden : false;
    elements.goalIntervalsContent.hidden = forceRefresh ? intervalsWereHidden : false;
  } finally {
    state.isLoadingGoalHalf = false;
    elements.showGoalHalf.disabled = !selectedFixture();
    elements.showGoalIntervals.disabled = !selectedFixture();
    elements.refreshGoalHalf.disabled = !selectedFixture();
    elements.refreshGoalIntervals.disabled = !selectedFixture();
    elements.showGoalHalf.textContent = elements.goalHalfContent.hidden ? "Mostrar" : "Ocultar";
    elements.showGoalIntervals.textContent = elements.goalIntervalsContent.hidden ? "Mostrar" : "Ocultar";
  }
}
...
async function loadYellowCards(forceRefresh = false) {
  ...
  const wasHidden = elements.yellowCardsContent.hidden;
  state.isLoadingYellowCards = true;
  elements.refreshYellowCards.disabled = true;
  elements.showYellowCards.disabled = true;
  elements.yellowCardsStatus.className = "status-badge status-badge--processing";
  elements.yellowCardsStatus.textContent = forceRefresh ? "Actualizando" : "Procesando";
  try {
    ...
    renderYellowCards({ status: "not_available", warning: error.message });
    elements.yellowCardsContent.hidden = forceRefresh ? wasHidden : false;
  } finally {
    state.isLoadingYellowCards = false;
    elements.showYellowCards.disabled = !selectedFixture();
    elements.refreshYellowCards.disabled = !selectedFixture();
    elements.showYellowCards.textContent = elements.yellowCardsContent.hidden ? "Mostrar" : "Ocultar";
  }
}
...
elements.refreshGoalIntervals.addEventListener("click", () => loadGoalHalf(true));
...
elements.clearPerformanceMonthFilter.addEventListener("click", () => {
  state.performanceMonthFilter = "";
...
elements.competitionPerformance.addEventListener("click", (event) => {
  const open = event.target.closest("[data-view-competition-picks]");
  if (open) showCompetitionPicksDialog(open.dataset.viewCompetitionPicks, open.dataset.competitionResult || "won");
});
