...
const WARNING = "xG estimado calculado internamente con estadísticas disponibles de API-Football. No corresponde a xG oficial de proveedor estadístico.";
...
function emptyResult(fixtureId, homeTeam, awayTeam, updatedAt, note = "No hay estadísticas suficientes para ambos equipos.") {
  return {
    status: "not_available", type: "estimated", source: "api-football-internal-model",
    modelVersion: MODEL_VERSION, fixtureId,
...
export function buildEstimatedXgFromDataset(dataset) {
  ...
  return {
    status: statusFromConfidence(score), type: "estimated", source: "api-football-internal-model",
    modelVersion: MODEL_VERSION, fixtureId: extracted.fixtureId,
    homeTeam: {
...
export async function getEstimatedXgForFixture(fixtureId, { loadFixtureDataset } = {}) {
  ...
  try {
    ...
    return {
      status: "failed", type: "estimated", source: "api-football-internal-model",
      modelVersion: MODEL_VERSION, fixtureId: String(fixtureId), homeTeam: null, awayTeam: null,
