...
function evaluateSide({ match, side, team, opponent, odds, now }) {
  ...
  if (contradictory || (!shotsMedium && !passesMedium) || team.players === null) return [];
  ...
  if (!reasonableAdvantage || (sample === "weak" && !bothStrong)) return [];
  ...
  const base = (market, selection, marketKey, selectionKey, explanation) => {
    ...
    return {
      ...
      marketKey, selectionKey, confidence, confidenceScore: weightedScore, color,
      ...
      canAdd: color !== "red", requiresReview: color !== "green",
      ...
      diagnostics: { shotsDiff, passesDiff, shotsLevel, passesLevel, disciplineAcceptable, sample, weightedScore }
    };
  };
