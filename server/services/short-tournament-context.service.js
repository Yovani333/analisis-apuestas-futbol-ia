...
export function buildShortTournamentContext(dataset = {}) {
  ...
  const isShortTournament = isWorldCup || /(^|\s)cup(\s|$)|copa|torneo corto/.test(competition);
  const phase = resolvePhase(dataset);
  const normalizedPhase = normalize(phase);
  const isKnockout = /round of|octavos|quarter|cuartos|semi|final|knockout|eliminacion/.test(normalizedPhase);
