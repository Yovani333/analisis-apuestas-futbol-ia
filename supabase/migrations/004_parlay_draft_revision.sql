...
begin
  ...
  on conflict (user_id) do update set
    parlay_draft = case
      when coalesce((excluded.preferences ->> 'parlayDraftUpdatedAt')::timestamptz, '-infinity'::timestamptz)
        >= coalesce((public.user_sync_state.preferences ->> 'parlayDraftUpdatedAt')::timestamptz, '-infinity'::timestamptz)
